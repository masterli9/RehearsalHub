...
export default function Band() {
  ...
  const handleMakeLeader = async (firebaseUid: string) => {
    try {
      ...
      Alert.alert("Error", error.message || "Failed to transfer leader role");
    }
  };
        ...
        Alert.alert(
          ...
          [
            ...
            },
            {
              text: "Leave",
              onPress: () => {
                handleRemoveMember(activeBand?.id || "", user?.uid || "");
              },
            },
          ],
        );
    ...
    <PageContainer noBandState={bands.length === 0}>
        ...
        >
          {({
            ...
            <>
              ...
              />
              {(touched.bandName || submitCount > 0) && errors.bandName && (
      ...
      >
        <Formik<JoinFormValues>
          ...
          onSubmit={async (values) => {
            try {
              await joinBandByCode(values.joinCode.toUpperCase(), values.roles);
        ...
        >
          {({
            ...
            <>
              ...
              />
              {(touched.joinCode || submitCount > 0) && errors.joinCode && (
        ...
        <Formik<EditBandNameFormValues>
          ...
          onSubmit={async (values) => {
            try {
              ...
              await updateBandName(activeBand.id, values.bandName);
              setShowEditBandNameModal(false);
              Alert.alert("Success", "Band name updated successfully");
            } catch (error: any) {
              Alert.alert(
                "Error",
                error.message ||
        ...
        >
          {({
            ...
            <>
              ...
              <View className="flex-row gap-4 w-full justify-center items-center my-3">
                <StyledButton
                  onPress={() => setShowEditBandNameModal(false)}
                  title="Cancel"
                />
        ...
        <Formik<EditRolesFormValues>
          ...
          onSubmit={async (values) => {
            try {
              ...
              const finalRoles = currentUserRoles.includes("Leader")
                ? [...values.roles, { role_id: 0, title: "Leader" }]
                : values.roles;
        ...
        >
          {({
            ...
            <>
              ...
              {(touched.roles || submitCount > 0) && errors.roles && (
                <ErrorText>
        ...
        <View className="flex-row flex-wrap gap-2 w-full justify-center items-center my-4">
          ...
          selectedMemberRoles.roles.length > 0 ? (
            selectedMemberRoles.roles
              ...
              .map((r: any, i: number) => (
                <Text
                  key={typeof r === "string" ? r : r.role_id || r.title || i}
      ...
      {bands.length === 0 ? (
        <>
          <Card className="flex-col items-center justify-center">
            ...
            <View className="flex-row gap-4 justify-center items-center my-3">
              <StyledButton
                title="Create a band"
        ...
        <>
          ...
          >
            <MenuOption
              ...
              customStyles={{
                optionText: {
                  color: systemScheme === "dark" ? "#fff" : "#333",
                  paddingVertical: 8,
                  fontSize: fontSize.base,
                },
              }}
            ...
            {isLoadingMembers && !refreshing ? (
              ...
              <>
                {bandMembers.map((member, idx) => (
                  <Card
                    key={
                      member.firebase_uid || member.id || member.email || idx
                    }
                      ...
                      member.firebase_uid !== user?.uid && (
                        <Menu>
                          ...
                          >
                            ...
                            <MenuOption
                              onSelect={() =>
                                handleMakeLeader(member.firebase_uid)
                              }
                              text="Make leader"
                              customStyles={{
                                optionText: {

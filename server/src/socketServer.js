...
export function setupSockets(server) {
    const io = new Server(server, {
        cors: {
            origin: "*", // Povolit vše z bezpečnostních důvodů (vyžadováno pro Pterodactyl a mobilní aplikace)
            credentials: true,
        },
    });
        ...
        socket.on("band:select", async ({ bandId }, ack) => {
            try {
                ...
                if (!Number.isInteger(id)) {
                    return ack?.({ ok: false, error: "invalid-band-id", tempId: undefined }); // fixed tempId reference
                }

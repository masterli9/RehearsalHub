        ...
        if (firebase_uid) {
            try {
                ...
                await logActivity(
                    bandIdInt,
                    userId,
                    `scheduled a ${type}: "${title}"`,
                    "event_scheduled",
                );

                // Odeslání push notifikace kapele
                const { sendPushNotificationToBand } =
                ...
                console.error(
                    "Failed to log event creation or send notification",
                    e,
                );
            }
        }
    ...
    try {
        ...
        const updateEventResult = await pool.query(
            ...
            [
                ...
                place || null,
                lengthValue,
                type === "concert" && setlist_id
                    ? parseInt(setlist_id, 10)
                    : null,
                id,
            ],
        );
        ...
        if (type === "rehearsal") {
            ...
            await pool.query("DELETE FROM event_songs WHERE event_id = $1", [
                event.event_id,
            ]);
...
export const getEvents = async (req, res) => {
    const { bandId, type, limit = 10, offset = 0 } = req.query;
